...
c: utilities
c+
	subroutine GetXY(tno,xyphase,sd,count,mxant,nants)
c
	implicit none
	integer tno,mxant,nants,count(mxant)
	real xyphase(mxant),sd(mxant)
c
...
c    sd		Standard deviation of the xyphase, in radians.
...
c------------------------------------------------------------------------
	include 'maxdim.h'
	include 'mirconst.h'
	integer item,iostat,ngains,nfeeds,ntau,nsols,i,j,k,off
	real theta
...
c
	do i=1,nants
	  if(count(i).gt.0)then
	    ...
	    xyphase(i) = theta
	    sd(i) = sqrt(abs(sd(i)/count(i) - theta*theta))
	  endif
	enddo
c
	end
