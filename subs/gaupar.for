...
c    29apr15 rjs  Handle Kelvin images with no beam parameters.
c
c************************************************************************
...
c
	  ...
	  bunit(l+1:) = '/'//b1b
	  if(bmaj1*bmin1.ne.0.and.bmaj2*bmin2.ne.0.and.
     *		dx1*dy1.ne.0)then
...
c
	  ...
	  bunit = bunit2x
	  if(bmaj1*bmin1.eq.0.or.dx1*dy1.eq.0)then
	    call bug('w','Bmaj or bmin missing ... no scaling')
	  else
	    if(bmaj2*bmin2.ne.0)then
	      call Gaufac(bmaj1,bmin1,bpa1,bmaj2,bmin2,bpa2,
     *		fac,bmaj,bmin,bpa,ifail)
	      ...
	      bmin = bmin1
	      bpa  = bpa1
	      fac  = 4.0*log(2.0)/PI * abs(dx1*dy1)/(bmaj*bmin)
	    endif
...
c
	  ...
	  bunit = bunit1x
	  if(bmaj2*bmin2.eq.0.or.dx1*dy1.eq.0)then
	    call bug('w','Bmaj or bmin missing ... no scaling')
	  else
	    if(bmaj1*bmin1.ne.0)then
	      call Gaufac(bmaj1,bmin1,bpa1,bmaj2,bmin2,bpa2,
     *		fac,bmaj,bmin,bpa,ifail)
