...
c@ region
c       Region of image to be used.  The default region is the entire
c       image.  See documentation of region for details.  All pixels
c       within the bounding box are used; pixel masking is ignored.
c@ center
c       The rotation center in arcsec from the center pixel, measured
c       in the directions of RA and DEC. Default=0,0.
c@ pa
c       Position angle of the major axis in degrees.  Default is 0
c       (north).
c@ incline
c       Inclination angle of the disk in degrees.  Default is 0 (face
c       on).
c@ radius
c       Sample points along major axis in arcsecs.  Need not be evenly
c       spaced.  At least one point must be given.
c@ vrot
c       Rotation curve in the plane of the disk specified by a power
c       law:
c               rotation velocity = vrot(1)*(r/radius(1))**vrot(2)
c       At least one point must be given.  For options=sample, vrot
c       gives the rotation velocity sampled at the radius points above,
c       and is interpolated between the sample points, with zero
c       velocity at zero radius, and not extrapolated beyond the last
c       point.  Units are km/s.
c@ vrad
...
c       Default=0,0.  If only one value given then z' = z(1).  The units
c       of z(1) are the same as arcsec along major axis of disk.
c@ out
...
c
      ...
      do while (keyprsnt('radius'))
        nrad = nrad+1
      call keyr('radius',radius(nrad),0.0)
      enddo
      nrot = 0
      do while (keyprsnt('vrot'))
        nrot = nrot+1
        call keyr('vrot',vrot(nrot),0.0)
      enddo
      call keyr('vrad',vrad(1),0.0)
      call keyr('vrad',vrad(2),0.0)
      call keyr('vz',vz,0.0)
      call keyr('vsys',vsys,0.0)
      call keyr('z',z0,0.0)
      call keyr('z',zexp,0.0)
      call keya('out',out,' ')
